...
def set_job(job_id: str, data: dict) -> None:
    redis_client.setex(f"speech:{job_id}", 600, json.dumps(data))
...
async def evaluate_speech(
    ...
    try:
        ...
        logger.error("Failed to read audio for job %s: %s", job_id, e)
        raise HTTPException(status_code=500, detail="Failed to read audio")

    # Store audio bytes in Redis (base64 encoded, TTL 10 min)
    # Key: speech:audio:{job_id}
    audio_b64 = base64.b64encode(contents).decode("utf-8")
    redis_client.setex(f"speech:audio:{job_id}", 600, audio_b64)

    # Write initial status
    set_job(job_id, {"status": "processing"})

    # Enqueue job — pass job_id and expected_text only, no file path
    queue.enqueue(
        process_job,
        job_id,
        expected_text,
